...
class YMF278Slot
{
	public:
		...
		byte_t bits;		// width of the samples
		int startaddr;
		int loopaddr;
		int endaddr;
...
class YMF278 : public SoundDevice
{
	public:
		...
		void writeRegOPL4(byte_t reg, byte_t data, const EmuTime &time);
		byte_t peekRegOPL4(byte_t reg, const EmuTime &time);
		byte_t readRegOPL4(byte_t reg, const EmuTime &time);
		byte_t peekStatus(const EmuTime &time);
		byte_t readStatus(const EmuTime &time);
	...
	private:
		byte_t readMem(unsigned int address);
		void writeMem(unsigned int address, byte_t value);
